<x-app-layout>
    <x-slot name="head">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Create Subjects</h1>
    </x-slot>
    ...
    <div class="glass-effect rounded-2xl p-8 shadow-2xl bg-white dark:bg-gray-900">
        ...
        <form action="{{ route('subject.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2"> {{-- 2 columns layout --}}
                        @foreach ($groupData as $group)
                            <label class="flex items-center space-x-2 p-2 border rounded-lg dark:border-gray-600">
                                <input type="checkbox" name="group_ids[]" value="{{ $group->id }}"
                                    {{ is_array(old('group_ids')) && in_array($group->id, old('group_ids')) ? 'checked' : '' }}
               ...
               rounded-xl focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:outline-none text-gray-900 dark:text-white" />
                ...
                <div class="mt-4">
                    ...
                    <input type="number" name="code" id="code" value="{{ old('code') }}" required
                        class="w-full px-4 py-3 bg-white dark:bg-gray-800 border
